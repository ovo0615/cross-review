...
def add_user_voice(add, parsed: dict, job: dict = None) -> None:
    ...
    if referenced:
        add("## 使用者這句話是在回應什麼")
        add()
        add("使用者這一輪的發言很短（或明顯在指涉先前的內容），單看它無法知道"
            "他授權執行的是哪些事。以下是**執行者上一則回覆**的內容，"
            "也就是使用者當時看著、然後做出回應的東西。")
        add()
        add("**使用者到底同意了哪些、否決了哪些、改了哪些，一律以上面他的原文為準。**"
            "這一段只是脈絡，不代表使用者同意了它的全部——他可能只採用其中幾項，"
            "也可能是在否決它。")
        add()
