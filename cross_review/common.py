...
# 預設是 **manual**。
#
# 這一條改過一次。工具原本是逐專案安裝的，「要不要讓這個專案的程式碼出去給
# 第三方模型」在安裝那一刻就已經表態過了，所以預設 threshold（有安全網、
# 不會因為忘記而漏審）是合理的。
#
# 改成全域安裝之後，那個表態的動作消失了：任何目錄開 session 都會被當成專案。
# 實測餵一個 %TEMP% 路徑給 hook，它照樣認成專案、掃出 41 KB 改動並要求送審。
# 而這台機器的 D:\ 底下有 BM\、Consultant Cases\、Customer issue\ 等客戶資料。
#
# 所以「同意送出」這個動作要移到 trigger：預設不送，使用者對某個專案說了才送。
# 忘記送審的代價是少一份審查；預設送出的代價是客戶的程式碼在沒有人點頭的
# 情況下離開這台機器。兩者不對稱。
